...
const STRINGS = {
  it: {
    ...
    landingSubMobile: "Scorri verso destra e scegli un genere per sfogliare le recensioni disponibili.",
    ...
    thresholdText: "Attenzione: entrambe le versioni qui sotto possono contenere spoiler pesanti, finale incluso.",
  ...
  en: {
    ...
    landingSubMobile: "Swipe right and choose a genre to browse the available reviews.",
    ...
    thresholdText: "Warning: both versions below may contain heavy spoilers, including the ending.",
...
function fillGenreList(genreName){
  ...
  items.forEach(item => {
    ...
    // durante il tocco): mancava del tutto prima, l'unica
    // evidenziazione esistente era ":active" (il lampo momentaneo
    // del tocco stesso), sparita subito dopo - stessa idea gia' in
    // uso sulle altre 3 pagine (game-list button.is-active).
...
function syncTableToReview(reviewId){
  ...
  // (evita un ricostruzione/dissolvenza inutile), ma la voce attiva
  // va comunque aggiornata alla nuova recensione appena aperta -
  // altrimenti restava segnata quella precedente, o nessuna la
  // prima volta che si apre una voce dello stesso genere gia' in
  // vista.
...
function posizionaTextSizeToggle(){
  if(!el.textSizeToggle || !el.musicControl || !el.stageControls || !el.gateSideToggle) return;
  if(isMobileNav()){
    ...
    if(el.textSizeToggle.parentElement !== el.stageControls){
      el.stageControls.insertBefore(el.textSizeToggle, el.gateSideToggle);
    }
  }
}
