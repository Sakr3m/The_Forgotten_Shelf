...
el.indexLink.addEventListener("click", (ev) => {
  if(state.view !== "landing"){
    ...
    // non e' piu' un numero indovinato (550ms, che a volte tagliava
    // comunque la coda del suono se la sua durata reale era
...
// normale della recensione (che qui non esiste ancora, la pagina si
// e' appena caricata) mostrando titolo/gioco salvati cosi' come sono;
// niente pulsante salta, non c'e' una scaletta nota in questo contesto.
(function resumePersistedMusic(){
  if(!el.bgMusic || !isMusicPersistOn()) return;
  let saved;
