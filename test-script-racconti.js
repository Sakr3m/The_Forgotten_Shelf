// ============================================================
// SOLO PER QUESTA PAGINA DI TEST: un "elemento fittizio" che prende
// il posto di null quando document.getElementById non trova
// qualcosa - assorbe qualunque lettura/scrittura di proprieta' o
// chiamata di metodo senza generare errore. Permette di eliminare
// elementi dall'HTML del test senza far crashare lo script su un
// riferimento senza controllo di sicurezza (es. el.trackSkipBtn.
// addEventListener(...) scritto fuori da una funzione, quindi
// eseguito subito all'apertura della pagina).
// ============================================================
function creaElementoFittizio(){
  const style = new Proxy({}, { get: () => "", set: () => true });
  const classList = { add(){}, remove(){}, toggle(){ return false; }, contains(){ return false; } };
  const noop = function(){ return creaElementoFittizio(); };
  const handler = {
    get(target, prop){
      ...
      if(prop === "parentNode" || prop === "parentElement" || prop === "nextSibling" || prop === "firstChild") return null;
      ...
      return noop; // per tutto il resto (addEventListener, setAttribute,
        // appendChild, play, pause, ecc.) una funzione vuota che non
        // fa nulla quando chiamata, cosi' niente puo' mai lanciare
        // "is not a function" o "cannot read property of null".
    },
...
document.getElementById = function(id){
  return _origGetElementById(id) || creaElementoFittizio();
};
const _origQuerySelector = document.querySelector.bind(document);
document.querySelector = function(sel){
  return _origQuerySelector(sel) || creaElementoFittizio();
};
