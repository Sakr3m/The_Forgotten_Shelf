...
Bloodborne, in the end, was never really a game about waking up. It's a game about continuing to dream, deeper and deeper, convincing yourself each time that you're one step closer to the exit.`
  ...
  chrono_trigger_ombra_di_lavos_in_jenova: {
    ...
    tracks: [
      ...
      { src: "https://pub-de8310383cdb437f8f0b585a6642e88e.r2.dev/Chrono%20Trigger%20Main%20Theme.mp3", title: "Main Theme", game: "Chrono Trigger" },
    ...
    body: {
      it: `Due parassiti cosmici, due giochi Square degli anni '90, e un sospetto che circola tra i fan da quasi trent'anni senza che nessuno sia mai riuscito a chiuderlo del tutto: Jenova, la calamità caduta dal cielo in Final Fantasy VII, e Lavos, il mostro sepolto sotto la crosta terrestre di Chrono Trigger, potrebbero appartenere alla stessa specie aliena. O peggio, essere frammenti della stessa entità originaria, la stessa idea riscritta due volte a distanza di due anni.

Le coincidenze, prese una per una, potrebbero anche essere casuali. Messe in fila, iniziano a somigliare a uno schema. Entrambi arrivano dallo spazio e si schiantano sul pianeta in epoche remote, dimenticate dalle stesse civiltà che li ospitano. Entrambi restano dormienti per millenni, accumulando energia mentre osservano il mondo sopra di loro crescere e prosperare, quasi fosse bestiame lasciato maturare prima del raccolto. Entrambi, quando vengono infine scoperti, sono scambiati per divinità: Lavos come un dio da placare con offerte e sacrifici, Jenova come un'antica Cetra caduta dal cielo, venerata prima ancora di essere compresa.

C'è poi la questione genetica, forse il punto più difficile da liquidare come coincidenza. Jenova può assimilare, mutare e riscrivere il materiale organico di chiunque entri in contatto con le sue cellule, dando origine ai cloni e alle mostruosità che popolano il gioco. Lavos fa qualcosa di sorprendentemente simile su scala ancora più ampia, incorporando il DNA delle specie che incontra per generare forme ibride che ripetono il proprio ciclo evolutivo. Non sembrano due minacce diverse quanto due varianti dello stesso comportamento: un parassita che non si limita a consumare un mondo, ma lo riscrive dall'interno, plasmandolo perché diventi più adatto a se stesso.

La parte più intrigante della teoria, però, non riguarda il testo dei due giochi, ma la storia dietro le quinte del loro sviluppo. Chrono Trigger uscì su Super Nintendo, una piattaforma con limiti tecnici severi e con le linee guida sui contenuti imposte da Nintendo all'epoca, molto più rigide di quelle che Square avrebbe poi trovato su PlayStation con Final Fantasy VII. L'ipotesi è che il team, chiuso dentro quei vincoli, non sia riuscito a dare a Lavos tutta la portata cosmica e orrorifica che avrebbe voluto davvero, e che quell'ambizione rimasta a metà sia riemersa due anni più tardi, libera da ogni restrizione, nella forma di Jenova. Non un sequel nascosto, quindi, ma la stessa paura ancestrale, quella di un'intelligenza aliena che usa i pianeti come incubatrici, realizzata due volte con mezzi diversi non appena i vincoli tecnici lo hanno permesso. Il fatto che diversi sviluppatori chiave abbiano lavorato su entrambi i titoli non fa che rendere l'ipotesi ancora più difficile da bollare come semplice suggestione.

Nessun collegamento è mai stato reso ufficiale, e probabilmente non lo sarà mai. Jenova funziona narrativamente proprio perché resta una minaccia incomprensibile, priva di un'origine chiara: legarla esplicitamente a un altro universo rischierebbe solo di sminuirla, di darle un'origine troppo semplice per qualcosa che dovrebbe restare oltre la comprensione umana. Ma come lettura non ufficiale, come eco silenziosa che un team di sviluppo si porta dietro da un progetto all'altro senza nemmeno accorgersene fino in fondo, la teoria regge sorprendentemente bene. Più che un frammento letterale della stessa creatura, Jenova sembra essere l'ombra che Lavos ha lasciato cadere sul progetto successivo, la versione che quel mostro avrebbe voluto essere fin dall'inizio.`,
      en: `Two cosmic parasites, two Square games from the '90s, and a suspicion that has circulated among fans for almost thirty years without anyone ever managing to fully close the case: Jenova, the calamity that fell from the sky in Final Fantasy VII, and Lavos, the monster buried beneath the Earth's crust in Chrono Trigger, might belong to the same alien species. Or worse, they might be fragments of the same original entity, the same idea rewritten twice two years apart.

Taken one by one, the coincidences could easily be dismissed as chance. Lined up together, they start to look like a pattern. Both arrive from space and crash into the planet in remote eras, forgotten by the very civilizations that go on to host them. Both remain dormant for millennia, gathering energy while watching the world above them grow and flourish, as if it were livestock left to mature before the harvest. Both, once finally discovered, are mistaken for gods: Lavos as a deity to be appeased with offerings and sacrifice, Jenova as an ancient Cetra fallen from the sky, worshipped before it was ever understood.

Then there's the genetic angle, perhaps the hardest point to wave away as coincidence. Jenova can assimilate, mutate, and rewrite the organic matter of anyone who comes into contact with its cells, giving rise to the clones and monstrosities that populate the game. Lavos does something strikingly similar on an even larger scale, incorporating the DNA of the species it encounters to generate hybrid forms that repeat its own evolutionary cycle. They don't look like two different threats so much as two variants of the same behaviour: a parasite that doesn't just consume a world, but rewrites it from the inside, reshaping it to better suit itself.

The most intriguing part of the theory, though, has nothing to do with either game's actual text, but with the story behind their development. Chrono Trigger came out on the Super Nintendo, a platform with severe technical limits and the strict content guidelines Nintendo enforced at the time, far stricter than what Square would later find on PlayStation with Final Fantasy VII. The hypothesis is that the team, boxed in by those constraints, couldn't give Lavos the full cosmic, horrific scope they truly wanted, and that unresolved ambition resurfaced two years later, free of every restriction, in the shape of Jenova. Not a hidden sequel, then, but the same ancestral fear, that of an alien intelligence using planets as incubators, realized twice through different means the moment the technical constraints allowed it. The fact that several key developers worked on both titles only makes the hypothesis harder to write off as mere suggestion.

No connection has ever been made official, and probably never will be. Jenova works narratively precisely because it remains an incomprehensible threat with no clear origin: tying it explicitly to another universe would only risk cheapening it, giving it too simple an origin for something meant to stay beyond human understanding. But as an unofficial reading, as a silent echo a development team carries from one project to the next without ever fully noticing it themselves, the theory holds up remarkably well. More than a literal fragment of the same creature, Jenova seems to be the shadow Lavos left behind on the project that followed, the version of itself that monster wanted to be all along.`
    }
  },
  clair_obscur_chi_e_quel_bambino: {
    game: "expedition33",
...
La cosa più strana è cosa succede a quella statua man mano che Gris recupera pezzi della propria voce. Non è un caso isolato: ogni volta che Gris impara a cantare un po' meglio, qualcosa nella pietra sembra rispondere, come se le due cose fossero collegate da un filo che il gioco non spiega mai a parole. Nell'istante in cui Gris, raccolte tutte le stelle di ogni mondo, sta per ascendere sul ponte di costellazioni finale, la creatura oscura la trascina in un mare di melma nera e minaccia di sommergerla del tutto: è proprio in quel momento che la statua torna in vita, ricomponendosi pezzo per pezzo e cantando insieme a lei per scacciare il mostro. Non arriva da nessuna parte, non viene evocata da un rituale o da un oggetto magico: si ricompone perché Gris canta, punto. Come se quella voce fosse sempre stata anche la sua.

Chi è, allora, questa presenza che si sgretola all'inizio e torna a cantare alla fine, che Gris sembra cercare in ogni rovina del mondo senza mai nominarla? Il gioco lo rivela solo a chi arriva fino in fondo, e solo a chi si prende la briga di raccogliere ogni singolo ricordo sparso per la mappa. Fatto questo, si sblocca una stanza segreta: una scena notturna, un prato silenzioso, una piccola Gris bambina accanto a sua madre, che tiene una lucciola tra le mani e la lascia volare via. È l'unico momento in cui quella donna ha davvero un volto, un gesto, un corpo che non sia pietra.

A quel punto ogni statua incontrata nel gioco cambia significato. Non sono più semplici elementi scenografici sparsi lungo il percorso: sono la stessa madre, ripetuta in ogni stato di rovina possibile, perché il lutto non è un singolo evento superato una volta per tutte, ma una presenza che continua a riaffiorare in forme diverse, capitolo dopo capitolo, mai davvero lasciata alle spalle. E il fatto che torni a cantare proprio quando Gris ritrova la propria voce suggerisce che quella statua non fosse mai stata solo un ricordo immobile da contemplare, ma una parte di Gris stessa, in attesa di essere lasciata libera di parlare di nuovo.

Il fatto che questa rivelazione sia del tutto opzionale, quasi nascosta apposta in fondo alla mappa, ha fatto discutere: c'è chi l'ha letta come un'aggiunta scomoda, come se il gioco tornasse indietro a chiudere ogni interpretazione più aperta del giocatore con un secco "no, è proprio lei." Ma si può leggerla anche al contrario: non una correzione, ma un premio per chi ha davvero cercato fino in fondo, esattamente come si cerca di ricomporre con precisione il volto di chi si è perso.

Alla fine, Gris non sconfigge il mostro da sola. Lo sconfigge insieme all'eco di sua madre, ricostruita pezzo per pezzo dalla propria voce ritrovata. Il lutto, in Gris, non si supera dimenticando chi non c'è più, ma imparando a farla rivivere nella propria voce, un frammento alla volta.`,
      en: `There's a woman of stone who appears in every corner of Gris's journey, and the game never gives her a name. Gris meets her for the first time in the palm of her hand, at the very start of everything: the statue is enormous, and it crumbles under the girl's weight, dropping her onto a colorless earth where fragments of the broken hand lie half-buried among the ruins. From then on, that same woman keeps reappearing along Gris's path, always different, always more ruined: in the desert she's carved into four pillars, her face pressed between her hands in poses of grief; in the flooded depths she returns enormous, her base turned into a staircase Gris has to climb. Every chapter, one way or another, puts her in front of Gris again.

The strangest part is what happens to that statue as Gris recovers pieces of her own voice. It's not an isolated case: every time Gris learns to sing a little better, something in the stone seems to respond, as if the two things were connected by a thread the game never explains in words. In the instant Gris, having gathered every star from every world, is about to climb the final constellation bridge, the dark creature drags her into a sea of black sludge and threatens to swallow her whole: it's exactly then that the statue comes back to life, reforming piece by piece and singing alongside her to drive the monster away. It doesn't come from anywhere; it isn't summoned by a ritual or a magic object. It reforms because Gris sings, full stop. As if that voice had always been hers too.

Who is, then, this presence that crumbles at the start and comes back singing at the end, that Gris seems to search for in every ruin of the world without ever naming her? The game only reveals it to those who make it all the way to the end, and only to those who bother collecting every single memento scattered across the map. Once that's done, a secret room unlocks: a nighttime scene, a quiet meadow, a young Gris standing beside her mother, who holds a firefly cupped in her hands and lets it fly free. It's the only moment that woman truly has a face, a gesture, a body that isn't stone.

From that point on, every statue encountered in the game changes meaning. They're no longer just scenery scattered along the path: they're all the same mother, repeated in every possible state of ruin, because grief isn't a single event you get past once and for all, but a presence that keeps resurfacing in different forms, chapter after chapter, never truly left behind. And the fact that she comes back singing exactly when Gris regains her own voice suggests that statue was never just a still memory to contemplate, but a part of Gris herself, waiting to be set free to speak again.

The fact that this reveal is entirely optional, almost deliberately tucked away at the far edge of the map, has sparked debate: some have read it as an uncomfortable addition, as if the game were stepping back in to shut down any more open interpretation the player had reached with a flat "no, it really is her." But you can read it the other way too: not a correction, but a reward for those who truly searched to the very end, the same way you search to recall, precisely, the face of someone you've lost.

In the end, Gris doesn't defeat the monster alone. She defeats it together with the echo of her mother, rebuilt piece by piece from her own rediscovered voice. Grief, in Gris, isn't overcome by forgetting the one who's gone, but by learning to bring her back to life in your own voice, one fragment at a time.`
    }
  }  ,
  hollow_knight_ricettacolo_perfetto: {
    game: "hollow_knight",
